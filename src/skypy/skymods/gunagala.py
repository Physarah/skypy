# DISCLAIMER: This code is copied from Gunagala, https://gunagala.readthedocs.io/en/develop/
# It has been edited and included here so that you can change the reddening parameter of the generated Zodi SED
...
class Simple(GunagalaSkyModel):
    ...
    def surface_brightness(self, filter_name):
        ...
        try:
            ...
            raise ValueError(
                "Sky model has no surface brightness value for filter '{}'!".format(filter_name))

        return sb


class ZodiacalLight(GunagalaSkyModel):
    """
    ...
    def __init__(self, _f_blue, _f_red, **kwargs):
        ...
        self._zl_normalisation = _zl_nep / _solar_normalisation
        # Central wavelength for reddening/normalisation
        self._lambda_c = 0.5 * u.um
        self._f_blue = _f_blue
        self._f_red = _f_red
        # Aldering reddening parameters
        # Parameters for the zodical light spatial dependence

        # Data from table 17, Leinert et al (1997).
        self._llsun = np.array([0, 5, 10, 15, 20, 25, 30, 35, 40, 45,
                                60, 75, 90, 105, 120, 135, 150, 165, 180])
        self._beta = np.array([0, 5, 10, 15, 20, 25, 30, 45, 60, 75])
        self._zl_scale = np.array([[np.NaN, np.NaN, np.NaN, 3140, 1610, 985, 640, 275, 150, 100],
        ...
        solar_path = get_package_filename_dir('data', 'sun_castelli.fits')
    ...
    def surface_brightness(self, **kwargs):
        ...
        interpolator = interp1d(self.waves, self.photon_sfd,
    ...
    def _calculate_spectrum(self, solar_path):
        ...
        # Reddening factor
        rfactor = np.where(sun_waves < self._lambda_c,
                           1.0 + self._f_blue * np.log(sun_waves/self._lambda_c),
                           1.0 + self._f_red * np.log(sun_waves/self._lambda_c))
        # Apply normalisation and reddening
        sfd = sun_sfd * self._zl_normalisation * rfactor
        # #DownWithErgs
        ...
        fudge = fudge.to(u.photon * u.second**-1 * u.m**-2 * u.um**-1,
    ...
    def _calculate_spatial(self):
        ...
        zl_scale = self._zl_scale / 77

        # Expand range of angles to cover the full sphere by using symmetry
        beta = np.array(np.concatenate(
            (-np.flipud(self._beta)[:-1], self._beta))) * u.degree
        llsun = np.array(np.concatenate((self._llsun, 360 -
                                         np.flipud(self._llsun)[1:-1]))) * u.degree
        ...
        zl_scale_c_interp = SmoothBivariateSpline(
            betas_c[good], llsuns_c[good], zl_scale_cflat[good])
